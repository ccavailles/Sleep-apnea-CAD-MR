...
outcome_clump <- semi_join(outcome, exposure1_dat, by = "SNP")

# Exposure SNPs not present in outcome
exp_snps_wo <- anti_join(exposure1_dat, outcome, by = "SNP")
...
mr_dat <- harmonise_data(exposure1_dat, outcome_dat, action = 2) %>% as_tibble() %>%
  mutate(
...
mrlap <- MRlap(exposure = exposure1_test,
               exposure_name = "exposure1",
...
# Standard MR
mr_res <- mr(mr_dat, method_list = c("mr_ivw_fe","mr_ivw_mre", "mr_egger_regression", "mr_weighted_median", "mr_weighted_mode"))
mr_res

## MR analysis 
mr_res_outlier <- mr(mr_dat_outlier, method_list = c("mr_ivw_fe","mr_ivw_mre", "mr_egger_regression", "mr_weighted_median", "mr_weighted_mode"))
mr_res_outlier
...
radial_res_outlier <- ivw_radial(radial_dat_outlier$sa.outcome, alpha = 0.05/nrow(radial_dat_outlier$sa.outcome)) 
